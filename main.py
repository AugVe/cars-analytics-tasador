...
def home():
    return {
        "estado": "En línea",
        "mensaje": "Bienvenido a la API de "
        "Tasación y Segmentación de Vehículos",
        "modelos_cargados": 4,
    }
...
def analizar_vehiculo(auto: AutoInput):
    try:
        # 1. Convertimos el JSON que llega de la web
        # a una tabla de Pandas (1 sola fila)
        df_input = pd.DataFrame([auto.caracteristicas])

        # 2. Alineación de Columnas (El guardián de seguridad)
        # Esto asegura que si el usuario manda pocas variables,
        # Pandas rellene el resto con ceros
        ...
        # Usamos el diccionario que armamos ayer
        # para traducir el número a texto
...
def detectar_ganga(oferta: AutoGangaInput):
    try:
        ...
        if diferencia > 2000:
            decision = "🔥 EXCELENTE OPORTUNIDAD: Comprar de inmediato. "
            "Margen de ganancia alto."
        elif diferencia > 0:
            decision = "✅ BUEN PRECIO: Ligeramente por debajo del mercado. "
            "Considerar compra."
        elif diferencia > -1500:
            decision = (
                "⚠️ PRECIO JUSTO/ALTO: Negociar a la baja antes de comprar."
            )
        else:
