...
with tab_tasacion:
    ...
    datos_tasa = formulario_datos("tasa")
    if st.button("Tasar ahora", key="btn_tasa"):
        resp = requests.post(
            f"{API_URL}/analizar_vehiculo",
            json={"caracteristicas": datos_tasa},
        )
        ...
        if resp.status_code == 200:
            ...
            if (
                segmento == "Deportivos Premium"
                and gama == "Gama Estándar / Baja"
            ):
                st.info(
                    "💡 **Oportunidad Detectada:** "
                    "Este vehículo tiene ADN de alto rendimiento "
                    "pero se valora como un modelo generalista. "
                    "Excelente para entusiastas "
                    "que buscan potencia a bajo costo."
                )
            elif (
                segmento == "SUVs y Vehículos Pesados"
                and gama == "Gama Alta / Premium"
            ):
                st.warning(
                    "🛡️ **Perfil de Seguridad:** "
                    "Vehículo posicionado en el sector premium; "
                    "requiere una inversión superior en "
                    "mantenimiento y seguros."
                )
            elif segmento == "Económicos Citadinos":
                st.success(
                    "✅ **Perfil Eficiente:** Ideal para movilidad urbana; "
                    "bajo costo operativo y alta liquidez en el mercado."
                )
            else:
                st.write(
                    f"El vehículo se clasifica como {segmento} "
                    "y se encuentra en {gama}."
                )
        else:
            st.error(
                "Ocurrió un error al consultar la tasación. "
...
with tab_gangas:
    ...
    precio_pub = st.number_input(
        "Precio publicado del vendedor",
        min_value=0,
        value=25000,
        key="ganga_precio",
    )
